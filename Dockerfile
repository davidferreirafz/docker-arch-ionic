...
RUN su user; cd download; wget https://aur.archlinux.org/cgit/aur.git/snapshot/android-sdk.tar.gz; tar -zxvf android-sdk.tar.gz; cd android-sdk; makepkg;
RUN exit;
...
RUN makepkg;
RUN exit;
RUN pacman -U download/android-platform-26/android-platform-26-8.0.0_r02-1-any.pkg.tar.xz --noconfirm
