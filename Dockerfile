...
RUN useradd -m -g users -s /bin/bash user && mkdir /opt/download && chown user -R /opt/download
USER user
RUN cd /opt/download && wget https://aur.archlinux.org/cgit/aur.git/snapshot/android-sdk.tar.gz && tar -zxvf android-sdk.tar.gz && cd android-sdk && makepkg && ls
USER root
